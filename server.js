...
const dbPath = process.env.NODE_ENV === 'production' 
    ? '/tmp/codechecker.db' 
    : path.join(__dirname, 'codechecker.db');
...
function initDatabase() {
    db.serialize(() => {
        ...
        // Таблица запросов (история проверок)
        ...
        const errorsKnowledge = [
            ...
            ['IndexError', 'Выход за границы списка.', 'Проверьте длину списка перед доступом к элементу.', 'https://docs.python.org/3/tutorial/datastructures.html'],
            ['KeyError', 'Ключ отсутствует в словаре.', 'Используйте метод get() или проверяйте наличие ключа.', 'https://docs.python.org/3/library/stdtypes.html'],
            ['ValueError', 'Некорректное преобразование типа.', 'Используйте try-except для обработки.', 'https://docs.python.org/3/tutorial/errors.html']
        ];
...
app.post("/check", (req, res) => {
    ...
    const tempFile = path.join(process.env.NODE_ENV === 'production' ? '/tmp' : __dirname, `temp_${Date.now()}.py`);
...
app.listen(PORT, () => { 
    ...
    console.log(`🌐 Продакшен режим: ${process.env.NODE_ENV === 'production' ? 'ДА' : 'НЕТ'}`);
});